...
- [2. 資料表定義](#2-資料表定義)
    ...
    - [2.5 payees (收款人主檔)](#25-payees-收款人主檔)
    - [2.6 payee_change_requests (收款人異動申請)](#26-payee_change_requests-收款人異動申請)
...
graph TD
    Profiles[profiles<br/>使用者資料]:::mainStyle
    Claims[claims<br/>請款單主表]:::mainStyle
    Items[claim_items<br/>請款明細]:::mainStyle
    History[claim_history<br/>審核歷程]:::subStyle
    Payments[payments<br/>付款單]:::subStyle
    Payees[payees<br/>收款人/廠商]:::mainStyle
    PayeeChanges[payee_change_requests<br/>收款人異動申請]:::subStyle
    Categories[expense_categories<br/>費用類別]:::subStyle

    %% 關聯關係
    Profiles -- "1:N<br/>申請人/核准人" --> Claims
    Claims -- "1:N" --> Items
    Claims -- "1:N" --> History
    Claims -- "N:1" --> Payments
    Claims -- "N:1<br/>廠商請款" --> Payees
    
    Items -- "N:1" --> Categories
    
    Profiles -- "1:N<br/>異動申請人/審核人" --> PayeeChanges
...
| `payee_id` | uuid | 外部收款人 | FK → payees.id (NULL 則為申請人本人) |
...
*(註：採方案 C 設計。員工個人報銷時 payee_id 為空；委外廠商或個人勞務則連結至收款人主檔)*
...
|-----|------|------|------|
| `id` | uuid | 主鍵 | |
| `type` | enum | 收款人類型 | vendor/personal |
| `name` | varchar(20) | 名稱 | 公司名或個人姓名 |
| `unified_no` | varchar(8) | 統一編號 | 明文儲存（廠商） |
| `national_id` | bytea | 身分證字號 | 加密儲存（個人） |
| `national_id_tail` | varchar(5) | 身分證末五碼 | 遮罩顯示用 |
...
### 2.6 payee_change_requests (收款人異動申請)

| 欄位 | 型別 | 說明 | 備註 |
|-----|------|------|------|
| `id` | uuid | 主鍵 | |
| `payee_id` | uuid | 收款人 | FK → payees.id (新增時為 null) |
...
| `proposed_unified_no` | varchar(8) | 提案中的統一編號 | 廠商明文 |
| `proposed_national_id` | bytea | 提案中的身分證字號 | 個人戶加密儲存 |
| `proposed_national_id_tail` | varchar(5) | 提案中的身分證末五碼 | 遮罩顯示用 |
...
| `payee_id` | uuid | 外部收款人 ID | FK → payees.id (NULL 則為員工本人) |
| `payee_name` | varchar(20) | 收款對象名稱 | 冗餘儲存以便快速檢視 |
...
-- (註：收款人異動均透過 payee_change_requests 流程，核准後由 Trigger/Function 自動更新 payees 表)
...
- `payees.national_id`, `payees.bank_account`
...
| Payees | `/payees/new` | `createPayeeRequest` | 提交新增收款人申請 |
| Payees | `/payees` | `updatePayeeRequest` / `submitDisableRequest` / `approvePayeeRequest` / `rejectPayeeRequest` / `withdrawRequest` | 收款人異動流程 |
| Payees | `/payees` | `revealPayeeAccount` / `revealPayeeTaxId` / `toggleStatus` / `removePayee` | 敏感欄位揭露與直接操作（僅 finance） |
| Admin | `/admin/users` | `updateUserPermissions` / `updateUserProfile` / `revealUserBankAccount` / `getUserProfileSnapshot` | 使用者管理：admin 與 finance 可檢視，僅 admin 可改角色與生命週期 |
...
- `20260215011500_split_payee_identity_and_unify_crypto_key.sql`

目的：

- 補齊主管在審核流程對下屬 profile 的讀取權限。
- 補齊財務角色對 profile 的讀取權限，避免審核/付款流程受限。
- 拆分收款人身分欄位：`unified_no`（明文）與 `national_id`（加密），並統一加密金鑰來源 `get_crypto_key()`。
...
- 最新執行結果：`npm run test:all:stable` → Vitest `15 passed`、Playwright `52 passed`。
