...
| **AccountPage** | `src/routes/account` | **個人帳務管理**：處理使用者基本資料、權限顯示及其加密的銀行帳號資訊。 | `UI` |
| **AdminUsers** | `src/routes/admin/users` | **使用者管理中樞**：管理員設定核准關係、調整權限與角色。 | `UI` |
...
4.  **步驟 2b：資料配發 (L_Server -> Layout)**：
    `+layout.server.ts` 透過 `locals.user` 取得已處理的使用者角色（admin, finance），並將 Profile 資料封裝進 `data` 傳回前端。
