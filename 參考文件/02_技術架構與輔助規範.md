...
│   ├── 📁 account/              ← 👤 個人帳戶 Action 端點
│   │   └── 📄 +page.server.ts  ← 處理設定更新與解密 RPC
│   ├── 📁 auth/               ← 🔑 認證相關路由
│   │   ├── 📄 +page.svelte    ← Google 登入頁面
│   │   └── 📁 callback/       ← OAuth 回傳處理
│   │       └── 📄 +server.ts  ← 處理登入成功後的代碼交換
│   ├── 📁 payees/             ← 🆕 受款人管理
│   │   ├── 📄 +page.svelte    ← 受款人列表 (含過濾與狀態)
│   │   ├── 📄 +page.server.ts ← 載入受款人與異動請求
│   │   └── 📁 new/            ← 新增受款人
│   │       ├── 📄 +page.svelte    ← 動態表單 (廠商/個人)
│   │       └── 📄 +page.server.ts ← 處理提交與加密 RPC
│
└── 📁 lib/                    ← ⭐ 共用程式碼庫
    ├── 📁 components/         ← 所有 UI 元件
    ...
    │   │
    │   ├── 📁 layout/         ← 🆕 佈局元件
    │   │   ├── 📄 Sidebar.svelte      ← 側邊導航 (含 RBAC 過濾與 Sheet 觸發)
    │   │   ├── 📄 UserAccountSheet.svelte ← 🆕 個人帳戶設定 (AES-256 解密)
