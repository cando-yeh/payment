-- Harden claims RPC permissions and storage policies.
...
REVOKE ALL ON FUNCTION public.create_claim(text, text, uuid, uuid, decimal, text, text, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_claim_detail(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.update_claim(varchar, text, uuid, decimal, text, text, text, text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.create_claim(text, text, uuid, uuid, decimal, text, text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_claim_detail(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_claim(varchar, text, uuid, decimal, text, text, text, text) TO authenticated;
...
WITH CHECK (
    ...
    AND EXISTS (
        ...
        WHERE c.id::text = split_part(name, '/', 1)
          ...
          AND c.status IN ('draft', 'pending_doc_review', 'paid_pending_doc', 'returned')
    )
);
        ...
        WHERE c.id::text = split_part(name, '/', 1)
          AND (
              ...
              OR EXISTS (
                  ...
                  FROM public.profiles applicant
                  WHERE applicant.id = c.applicant_id
                    AND applicant.approver_id = auth.uid()
              )
          )
    )
);
