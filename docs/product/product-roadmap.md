...
**A. 憑證識別與命名規則**
- **憑證識別**：每筆交易明細 (Line Item) 以 `claim_items.extra` 內的 `file_path` 作為主要識別。
...
**B. 憑證與交易紀錄綁定**
- 每筆交易明細記錄包含：
  - `extra.file_path`：Storage 路徑
  - `extra.file_name`：檔名
  - `extra.mime_type`：檔案型別
  - `attachment_status`：`uploaded` / `pending_supplement` / `exempt`
- 前端顯示時透過受保護下載 API 取得檔案 URL。

**C. 憑證生命週期管理**
| 使用者操作 | 後台行為 |
| :--- | :--- |
| **上傳新憑證** | 儲存檔案，更新 `extra.file_path/file_name/mime_type` |
| **更換憑證** | 刪除舊檔案，上傳新檔案，更新 `extra` 內容 |
...
- [x] **5.6 憑證管理**
  - [x] 檔案上傳 (PDF / JPG / PNG)
  - [x] 檔案大小驗證 (< 10MB)
  - [x] 儲存附件 metadata（`extra.file_path/file_name/mime_type`）
